...
class LoadRGBDImageFromFile:
    ...
    def __call__(self, results):
        ...
        # h, w = img.shape[:2]
        # filenamed = filename.replace('images', 'dhh')[:-4] + '.bin'
        # imgd = np.fromfile(filenamed, dtype=np.float16)
        # imgd.shape = h, w, 3

        # filenamed = filename.replace('images', 'hha')
        # filenamed = filename.replace('images', 'jet_2')
        # filenamed = filename.replace('images', 'dda')
        # filenamed = filename.replace('images', 'norm')
        filenamed = filename.replace('images', 'hhanew')

        imgd = mmcv.imread(filenamed, flag='unchanged')
...
class LoadRGBDImageFromFile1:
    ...
    def __call__(self, results):
        ...
        filename = filename.replace('images', 'imageso')
        ...
        # filenamed = filename.replace('images', 'dda')
        # filenamed = filename.replace('images', 'norm')
        # filenamed=filename.replace('imageso','hhanewo')
        filenamed = filename.replace('imageso', 'hhanewo')
...
class LoadRGBDAnnotations:
    ...
    def _load_bbox3d(self, results):
        results['gt_dims'] = results['ann_info']['dims'].copy()
        results['gt_locs'] = results['ann_info']['locs'].copy()
        results['gt_rots'] = results['ann_info']['rots'].copy()
        return results
    ...
    def __call__(self, results):
        ...
        if self.with_bbox3d:
            results = self._load_bbox3d(results)
        if self.with_mask:
