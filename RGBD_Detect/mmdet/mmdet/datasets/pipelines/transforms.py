...
class NormalizeRGBD:
    ...
    def __init__(self, mean, std, dmean, dstd, to_rgb=True):
        self.mean = np.array(mean, dtype=np.float32)
        self.std = np.array(std, dtype=np.float32)
        self.dmean = np.array(dmean, dtype=np.float32)
        self.dstd = np.array(dstd, dtype=np.float32)
        self.to_rgb = to_rgb
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if key == 'img':
                imgtmp = results[key]
                if 'imgmaskp' in results:
                    imgtmpmask = results['imgmaskp']
                else:
                    imgtmpmask = (imgtmp[:, :, 0] == 0) & (imgtmp[:, :, 1] == 0) & (imgtmp[:, :, 2] == 0)
                imgtmp = mmcv.imnormalize(imgtmp, self.mean, self.std,
                                          self.to_rgb)
                imgtmp[imgtmpmask] = 0
                results[key] = imgtmp
            ...
            if key == 'imgd':
                ...
                if len(imgd.shape) == 2:
                    ...
                    imgd[imgd < 100] = 0
                    imgd = imgd / 7500.0
                    results[key] = imgd.astype(np.float32)
                else:
                    imgd = results[key]
                    idex = (imgd[:, :, 0] == 0) & (imgd[:, :, 1] == 0) & (imgd[:, :, 2] == 0)
                    imgd = mmcv.imnormalize(imgd, self.dmean, self.dstd, True)
                    imgd[idex] = 0
                    results[key] = imgd
...
@PIPELINES.register_module()
class PhotoMetricDistortionRGBD:
    ...
    def __call__(self, results):
        ...
        img = results['img']
        idextmp = (img[:, :, 0] == 0) & (img[:, :, 1] == 0) & (img[:, :, 2] == 0)
        results['imgmaskp'] = idextmp
    ...
    def __call__(self, results):
        ...
        while True:
            ...
            for i in range(20):
                ...
                imgd = imgd[patch[1]:patch[3], patch[0]:patch[2]]
    ...
    def __call__(self, results):
        ...
        if np.all(self.mean == self.mean[0]):
            expand_img = np.empty((int(h * ratio), int(w * ratio), c),
                                  img.dtype)
            expand_imgd = np.empty((int(h * ratio), int(w * ratio), c),
                                   imgd.dtype)

            expand_img.fill(self.mean[0])
            expand_imgd.fill(self.mean[0])
        else:
            expand_img = np.full((int(h * ratio), int(w * ratio), c),
                                 self.mean, dtype=img.dtype)
            expand_imgd = np.full((int(h * ratio), int(w * ratio), c),
                                  self.mean, dtype=img.dtype)

        left = int(random.uniform(0, w * ratio - w))
        top = int(random.uniform(0, h * ratio - h))
        expand_img[top:top + h, left:left + w] = img
        expand_imgd[top:top + h, left:left + w] = imgd

        results['img'] = expand_img
        results['imgd'] = expand_imgd
        # expand bboxes
        for key in results.get('bbox_fields', []):
...
class MosaicRGBD:
    ...
    def _mosaic_transform(self, results):
        ...
        if len(results['img'].shape) == 3:
            ...
            mosaic_img_d = np.full(
                (int(self.img_scale[0] * 2), int(self.img_scale[1] * 2), 3),
                0,
                dtype=results['imgd'].dtype)
        else:
            mosaic_img = np.full(
        ...
        for i, loc in enumerate(loc_strs):
            ...
            img_i_d = mmcv.imresize(
                img_i_d, (int(w_i * scale_ratio_i), int(h_i * scale_ratio_i)))
            ...
            mosaic_img_d[y1_p:y2_p, x1_p:x2_p] = img_i_d[y1_c:y2_c, x1_c:x2_c]
