...
class CrossFusionandSENet(nn.Module):
    def __init__(self, channels_in_rgb, channels_in_depth, activation=nn.ReLU(inplace=True)):
        super(CrossFusionandSENet, self).__init__()

        self.se = SqueezeAndExcitation(channels_in_rgb, reduction=reducelist[channels_in_rgb],
                                       activation=activation)

        self.contactconv1 = nn.Conv2d(2 * channels_in_rgb, channels_in_rgb, kernel_size=(1, 1), groups=channels_in_rgb,
                                      bias=False)
        self.contactconv1_bn = nn.BatchNorm2d(channels_in_rgb)
        self.contactconv1_relu = nn.ReLU()

        self.contactconv2 = nn.Conv2d(channels_in_rgb, channels_in_rgb // reducelist[channels_in_rgb],
                                      kernel_size=(3, 3), padding=(1, 1),
                                      groups=channels_in_rgb // reducelist[channels_in_rgb], bias=False)
        self.contactconv2_bn = nn.BatchNorm2d(channels_in_rgb // reducelist[channels_in_rgb])
...
class FusionNetChange(BaseModule):
    def __init__(self,
                 init_cfg=None
                 ):
        super(FusionNetChange, self).__init__(init_cfg)
        ...
        self.se_layer0 = CrossFusionandSENet(64, 64)
        self.se_layer1 = CrossFusionandSENet(256, 256)
        self.se_layer2 = CrossFusionandSENet(512, 512)
        self.se_layer3 = CrossFusionandSENet(1024, 1024)
        self.se_layer4 = CrossFusionandSENet(2048, 2048)
    ...
    def forward(self, x):
        ...
        # fuse, rgb, depth = self.se_layer0(rgb, depth)
    ...
    def train(self, mode=True):
        super(FusionNetChange, self).train(mode)
        self._freeze_stages()
